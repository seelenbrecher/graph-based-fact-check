...
class inference_model(nn.Module):
    def __init__(self, bert_model, concept_model, args):
        ...
        if self.use_concept:
            ...
            self.rel_dim = args.relation_dim
            ...
            self.rel_important_proj = nn.Sequential(
                Linear(self.rel_dim, 1)
            )
        
        # GAT on span-level representation
        self.span_use_gat = args.span_use_gat
        if self.span_use_gat:
            self.span_gat = GAT(args, self.rel_important_proj)
    ...
    def self_attention(self, inputs, inputs_hiddens, mask, mask_evidence, adj_mat_all, index):
        ...
        adj_mat = torch.index_select(adj_mat_all, 1, idx).view(-1, self.max_len, self.max_len)
        _, adj_mat_embeds = self.concept_model(None, adj_mat)
    ...
    def get_intersect_matrix(self, q_embed, d_embed, attn_q, attn_d, rel_embeds):
        attn_q = attn_q.view(attn_q.size()[0], attn_q.size()[1], 1)
        attn_d = attn_d.view(attn_d.size()[0], 1, attn_d.size()[1], 1)
        
        # similarity score = cosine similarity * relation's important level
        sim = torch.bmm(q_embed, torch.transpose(d_embed, 1, 2)).view(q_embed.size()[0], q_embed.size()[1], d_embed.size()[1], 1)
        rel_important_scores = self.rel_important_proj(rel_embeds)
        sim = sim * rel_important_scores
    ...
    def get_intersect_matrix_att(self, q_embed, d_embed, attn_q, attn_d, adj_mat_embeds):
        """
        ...
        attn_d = attn_d.view(attn_d.size()[0], 1, attn_d.size()[1], 1)
        sim = torch.bmm(q_embed, torch.transpose(d_embed, 1, 2)).view(q_embed.size()[0], q_embed.size()[1], d_embed.size()[1], 1)
        rel_important_scores = self.rel_important_proj(adj_mat_embeds)
        sim = sim * rel_important_scores
        pooling_value = torch.exp((- ((sim - self.mu.cuda()) ** 2) / (self.sigma.cuda() ** 2) / 2)) * attn_d
    ...
    def initialize_node(self, token_level_bert, t2c_tensor, tpool_tensor, graph_tensor):
        ...
        edge_indices, rel_indices, graph_inp_mask, adj_mat, _  = graph_tensor
        ...
        adj_mat = adj_mat.view(-1, self.max_len, self.max_len)

        # get concept representation
        token_level_concept, rel_embeddings = self.concept_model(t2c_tensor, rel_indices)
        _, adj_mat_rel_embeds = self.concept_model(None, adj_mat)

        # v2 MAIN DIFF
        # use gat to get finer concept representation
        if self.span_use_gat:
            token_level_concept, _, _ = self.span_gat((token_level_concept, edge_indices, rel_embeddings))
        ...
        return token_level_bert, rel_embeddings, adj_mat_rel_embeds
    ...
    def forward(self, inputs):
        ...
        inputs_hiddens, rel_embeds, adj_mat_rel_embeds = self.initialize_node(inputs_hiddens, comb_inp_tensor, 
                                                                              tpool_tensor, graph_tensor)
...
#         mask_evidence = seg_tensor.float() * mask_text
        ...
        log_pooling_sum = self.get_intersect_matrix(inputs_hiddens_norm, inputs_hiddens_norm, mask_claim, 
                                                    mask_evidence, adj_mat_rel_embeds)
        ...
        inputs_att_de = []
        adj_mat_all = graph_tensor[-1]
        for i in range(self.evi_num):
            outputs, outputs_de = self.self_attention(inputs, inputs_hiddens, mask_text, mask_text, adj_mat_all, i)
...
class GAT(torch.nn.Module):
    ...
    def __init__(self, args, rel_important_proj):
        super().__init__()
        ...
        for i in range(self.num_of_layers - 1): #until -1 because I don't want to include the prediction layers. only the extractor layers
            layer = GATLayer(
                ...
                rel_important_proj=rel_important_proj,
            )
...
class GATLayer(torch.nn.Module):
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, concat=True, activation=nn.ELU(),
        ...
        self.rel_important_proj = rel_important_proj # linear projection to get relation's important level
    ...
    def forward(self, data):
        ...
        #
        in_nodes_features, edge_index, rel_embeds = data  # unpack data
        ...
        rel_important_scores = self.rel_important_proj(rel_embeds)
        scores_per_edge = self.leakyReLU((scores_source_lifted + scores_target_lifted) * rel_important_scores)
        ...
        return (out_nodes_features, edge_index, rel_embeds)
