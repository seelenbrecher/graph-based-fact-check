...
def tok2int_sent(sentence, tokenizer, max_seq_length, graph_constructor, helpers):
    """Loads a data file into a list of `InputBatch`s."""
    sent_a, title, sent_b, c_concepts, e_concepts = sentence
    tokens_a = tokenizer.tokenize(sent_a)
    # helpers for generating graph between nodes. Append merged_tokens and concepts to this variable helpers[0] and helpers[1] resp.
    ...
    helpers[0].append(merged_tokens)
    helpers[1].append(tok2concept)
    ...
    head_indices, tail_indices, rel_ids, adj_mat = graph_constructor.construct_concept_graph(merged_tokens, tok2concept, 
                                                                                         comb_input_mask)
    
    bert_input = (input_ids, input_mask, segment_ids)
    combine_input = (tok2concept, comb_input_mask, comb_segment_ids)
    graph_input = (head_indices, tail_indices, rel_ids, adj_mat)
...
def tok2int_list(src_list, tokenizer, max_seq_length, graph_constructor, max_seq_size=-1):
    ...
    # this one for generating graph between nodes
    merged_tokens = []
    concepts = []
    helpers = (merged_tokens, concepts)
    ...
    adj_mat_padding = list() # adjmat for a node w.r.t to itself
    
    tok_pool_mask_padding = list()
    for step, sent in enumerate(src_list):
        bert_input, combine_input, graph_input, tok_pool_mask = tok2int_sent(sent, tokenizer, max_seq_length, graph_constructor, helpers)
        input_ids, input_mask, input_seg = bert_input
        tok2concept, comb_mask, comb_seg = combine_input
        head_indices, tail_indices, rel_ids, adj_mat = graph_input
        ...
        adj_mat_padding.append(adj_mat)
        
        tok_pool_mask_padding.append(tok_pool_mask)
    
    adj_mat_interevis = list() #adjmat for a node w.r.t to all the other nodes\
    for i in range(max_seq_size):
        adj_mat_tmp = []
        for j in range(max_seq_size):
            if i >= len(src_list) or j >= len(src_list): # some kind of padding
                adj_mat_tmp.append([[REL_DUMMY_IDX] * max_seq_length for i in range(max_seq_length)])
            else:
                tokens_a, concepts_a = helpers[0][i], helpers[1][i]
                tokens_b, concepts_b = helpers[0][j], helpers[1][j]
                adjmat = graph_constructor.construct_graph_inter_evi(tokens_a, concepts_a, tokens_b, concepts_b)
                adj_mat_tmp.append(adjmat)
        adj_mat_interevis.append(adj_mat_tmp)
    if max_seq_size != -1:
        ...
        adj_mat_padding += ([[[REL_DUMMY_IDX] * max_seq_length] * max_seq_length] * (max_seq_size - len(tok_pool_mask_padding)))
    ...
    graph_inp_padding = (head_indices_padding, tail_indices_padding, rel_ids_padding, (adj_mat_padding, adj_mat_interevis))
...
class DataLoader(object):
    ...
    def next(self):
        ...
        if self.step < self.total_step:
            ...
            edge_idx_inputs, rel_idx_inputs, adj_mat_own_inputs, adj_mat_all_inputs = [], [], [], []
            tpool_inputs = []
            for step in range(len(inputs)):
                ...
                head_idx, tail_idx, rel_idx, adj_mat = graph_inp
                adj_mat_own, adj_mat_all = adj_mat
                ...
                adj_mat_own_inputs += adj_mat_own
                adj_mat_all_inputs += adj_mat_all
            ...
            max_len = graph_inp_add_padding(edge_idx_inputs, rel_idx_inputs) # max num of relations
            ...
            adj_mat_own_tensor_input = Variable(
                torch.LongTensor(adj_mat_own_inputs)).view(-1, self.evi_num, self.max_len, self.max_len)
            adj_mat_all_tensor_input = Variable(
                torch.LongTensor(adj_mat_all_inputs)).view(-1, self.evi_num, self.max_len, self.max_len)
            ...
            if self.cuda:
                ...
                adj_mat_own_tensor_input = adj_mat_own_tensor_input.cuda()
                adj_mat_all_tensor_input = adj_mat_all_tensor_input.cuda()
            ...
            graph_inp_tensor = edge_idx_tensor_input, rel_idx_tensor_input, rel_msk_tensor_input, adj_mat_own_tensor_input, \
                            adj_mat_all_tensor_input
