...
def load_transe_emb(args):
    ...
    rel_emb_mean = rel_emb.mean(dim=0)
    rel_emb_mean = rel_emb_mean.view(1, -1)
    rel_emb = torch.cat((rel_emb, rel_emb_mean), dim=0) # for semantic edges
    rel_emb = torch.cat((rel_emb, rel_emb_mean), dim=0) # for dummy edges
...
class GraphConstructor():
    ...
    def _make_connection_inter_evi(self, head_info, tail_info):
        """
        check if head and tail create an edge to the graph
        3 criteria:
            1. has relation w.r.t their concepts
            2. concept exact matching (TODO: can employ better matching)
        """
        blacklisted = ['[SEP]', '[CLS]']

        head, c_head = head_info
        tail, c_tail = tail_info
    ...
    def construct_concept_graph(self, tokens, concepts, input_mask):
        ...
        tail_indices = []
        rel_ids = []
        adj_mat = [[REL_DUMMY_IDX] * self.max_len for i in range(self.max_len)]
        for idx_head, head in enumerate(tokens):
            for idx_tail, tail in enumerate(tokens):
                ...
                if is_connect:
                    ...
                    rel_id = -1
                    if conn in self.rel2id:
                        rel_id = self.rel2id[conn]
                    elif conn == self.EXACT or conn == self.OCCURENCE:
                        rel_id = REL_SEMANTIC_IDX
                    else:
                        continue
                    rel_ids.append(rel_id)
                    adj_mat[idx_head][idx_tail] = rel_id
        return head_indices, tail_indices, rel_ids, adj_mat
    
    def construct_graph_inter_evi(self, tokens_a, concepts_a, tokens_b, concepts_b):
        n_token_a = len(tokens_a)
        n_token_b = len(tokens_b)
        ...
        for idx_head, head in enumerate(tokens_a):
            for idx_tail, tail in enumerate(tokens_b):
                c_head = None if concepts_a[idx_head] == -1 else self.id2concept[concepts_a[idx_head]]
                c_tail = None if concepts_b[idx_tail] == -1 else self.id2concept[concepts_b[idx_tail]]
                is_connect, conn = self._make_connection_inter_evi((head, c_head), (tail, c_tail))
                if is_connect:
                    rel_id = -1
                    if conn in self.rel2id:
                        rel_id = self.rel2id[conn]
                    elif conn == self.EXACT:
                        rel_id = REL_SEMANTIC_IDX
                    else:
                        continue
                    adj_mat[idx_head][idx_tail] = rel_id
