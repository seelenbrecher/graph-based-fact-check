...
def train_model(model, ori_model, args, trainset_reader, validset_reader):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for index, data in enumerate(trainset_reader):
            inputs, sc_inputs, lab_tensor = data
            prob, all_probs = model(inputs, sc_inputs)
            loss = loss_criterion.calculate(prob, all_probs, lab_tensor)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--infermodel_pretrain')
    ...
    parser.add_argument('--label_smoothing_ratio', default=1.0, type=float, help='ratio to smooth REFUTES and NEI label')
