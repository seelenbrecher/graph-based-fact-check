...
class inference_model(nn.Module):
    def __init__(self, bert_model, concept_model, args):
        ...
        self.input_proj = nn.Linear(self.node_dim * 2, self.node_dim)
    ...
    def get_intersect_matrix(self, q_embed, d_embed, attn_q, attn_d):
        ...
        pooling_value = sim * attn_d
        pooling_sum = torch.sum(pooling_value, 2)
        log_pooling_sum = pooling_sum * attn_q
    ...
    def get_relation_features(self, rel_inp_ids, segments, rel_indices):
        ...
        masks = torch.ones((bs, inp_len), dtype=torch.long).cuda()
        
        if self.roberta:
            out = self.pred_model(rel_inp_ids, masks)
            rel_emb = out.last_hidden_state
        else:
            rel_emb, _ = self.pred_model(rel_inp_ids, masks, segments)
        rel_emb = F.normalize(rel_emb, p=2, dim=2)
        
        rel_features = rel_emb * segments.unsqueeze(-1)
    ...
    def initialize_node(self, token_level_bert, t2c_tensor, tpool_tensor, graph_tensor):
        ...
        rel_indices = rel_indices.masked_fill_((1-graph_inp_mask).bool(), rel_inp_ids.shape[0] - 1)
    ...
    def forward(self, inputs, test=False, roberta=False):
        ...
        valid_evi = inp_tensor.sum(dim=2)>0
        valid_evi.requires_grad = False
...
#         mask_evidence = seg_tensor.float() * mask_text
        ...
        evi_prob = F.sigmoid(log_pooling_sum).squeeze(-1)
        
        inputs_hiddens_pool = inputs_hiddens.mean(dim=1)
        ...
        #inputs_att = ((inputs_att * evi_prob.unsqueeze(-1)) * valid_evi.unsqueeze(-1)).sum(dim=1) / valid_evi.sum(dim=1).unsqueeze(-1)
        inputs_att = ((inputs_att * evi_prob.unsqueeze(-1))).mean(dim=1)
        inference_feature = self.proj_inference_de(inputs_att)
        class_prob = F.softmax(inference_feature, dim=1)
#         prob = torch.sum(select_prob * class_prob, 1)
        prob = torch.log(class_prob)
