...
def add_concept_args(parser):
    ...
    parser.add_argument('--relation_num', default=37, type=int)
...
def get_rel_inputs_v1(tokenizer, rel_idx_inputs):
    relations = ["Antonym","AtLocation","CapableOf","Causes","CausesDesire","CreatedBy","DefinedAs","DerivedFrom","Desires","DistinctFrom","EtymologicallyDerivedFrom","EtymologicallyRelatedTo","FormOf","HasA","HasContext","HasFirstSubevent","HasLastSubevent","HasPrerequisite","HasProperty","HasSubevent","InstanceOf","IsA","LocatedNear","MadeOf","MannerOf","MotivatedByGoal","NotCapableOf","NotDesires","NotHasProperty","PartOf","ReceivesAction","RelatedTo","SimilarTo","SymbolOf","Synonym","UsedFor","IsSame"]

#     relation_to_sent_mapping = {'RelatedTo': 'is related to', 'FormOf': 'is a form of',
#                             'IsA': 'is', 'PartOf': 'is a part of', 'HasA': 'is a part of',
#                             'UsedFor': 'is used for', 'CapableOf': 'is able to',
#                             'AtLocation': 'is located at', 'Causes': 'is caused by',
#                             'HasSubevent': 'occurs as part of', 'HasFirstSubevent': 'begins with',
#                             'HasLastSubevent': 'concludes with', 'HasPrerequisite': 'needs',
#                             'HasProperty': 'is', 'MotivatedByGoal': 'is motivated by',
#                             'ObstructedBy': 'can be prevented by', 'Desires': 'want',
#                             'CreatedBy': 'created by', 'Synonym': 'is similar to',
#                             'Antonym': 'is different from', 'DistinctFrom': 'is distinct from',
#                             'DerivedFrom': 'is derived from', 'SymbolOf': 'is a symbol of',
#                             'DefinedAs': 'is defined as', 'MannerOf': 'is a',
#                             'LocatedNear': 'is located near',
#                             'HasContext': 'has context', 'SimilarTo': 'is similar to',
#                             'EtymologicallyRelatedTo': 'has a common origin with',
#                             'EtymologicallyDerivedFrom': 'is derived from',
#                             'CausesDesire': 'makes to want to', 'MadeOf': 'is made of',
#                             'ReceivesAction': 'can be', 'ExternalURL': 'External URL',
#                             'NotDesires': 'does not want', 'InstanceOf': 'is instance of',
#                             'subClassOf': 'is','NotCapableOf': 'is not able to','NotHasProperty': 'is not',
#                             'IsSame': 'is the same as'}

    relation_to_sent_mapping = {'RelatedTo': 'related to', 'FormOf': 'form of',
                            'IsA': 'is a', 'PartOf': 'part of', 'HasA': 'has a',
                            'UsedFor': 'used for', 'CapableOf': 'capable of',
                            'AtLocation': 'at location', 'Causes': 'causes',
                            'HasSubevent': 'has subevent', 'HasFirstSubevent': 'has first subevent',
                            'HasLastSubevent': 'has last subevent', 'HasPrerequisite': 'has prerequisite',
                            'HasProperty': 'has property', 'MotivatedByGoal': 'motivated by goal',
                            'ObstructedBy': 'obstructed by', 'Desires': 'desires',
                            'CreatedBy': 'created by', 'Synonym': 'synonym',
                            'Antonym': 'antonym', 'DistinctFrom': 'distinct from',
                            'DerivedFrom': 'derived from', 'SymbolOf': 'symbol of',
                            'DefinedAs': 'defined as', 'MannerOf': 'manner of',
                            'LocatedNear': 'located near',
                            'HasContext': 'has context', 'SimilarTo': 'similar to',
                            'EtymologicallyRelatedTo': 'etymologically related to',
                            'EtymologicallyDerivedFrom': 'etymologically derived from',
                            'CausesDesire': 'causes desire', 'MadeOf': 'made of',
                            'ReceivesAction': 'receives action', 'ExternalURL': 'external url',
                            'NotDesires': 'not desires', 'InstanceOf': 'instance of',
                            'subClassOf': 'sub class of','NotCapableOf': 'not capable of','NotHasProperty': 'not has property',
                            'IsSame': 'is same'}
    ...
    rels_to_id = {}
    cnt = 0
    for rel_ids in rel_idx_inputs:
        for rels in rel_ids:
            if str(rels) not in unique_rel:
                unique_rel[str(rels)] = rels
                rels_to_id[str(rels)] = cnt
                cnt += 1

    max_len = 0
    for rel_str in unique_rel:
        rels = unique_rel[rel_str]
        if rels[0] == -1:
            ids = ['[CLS]']
            ...
            input_ids.append([0]) # gak papa 1 dulu. di line berikutnya ntar di pad
            segments.append([0])
            continue
        ...
        for rel in rels:
            if rel != -1:
                sent = relation_to_sent_mapping[relations[rel]]
    ...
    for ids, s in zip(input_ids, segments):
        if len(ids) < max_len:
    ...
    for rel_ids in rel_idx_inputs:
        inputs = []
        for rels in rel_ids:
            inputs.append(rels_to_id[str(rels)])
        new_rel_idx_inputs.append(inputs)
    ...
    if input_ids == [] and len(input_ids) == 0:
        ...
        input_ids.append([0])
        segments.append([0])
    else: # add dummy relations
        try:
            input_ids.append([0] * len(input_ids[-1]))
            segments.append([0] * len(segments[-1]))
        except:
            import pdb
            pdb.set_trace()
    return input_ids, segments, new_rel_idx_inputs