...
class GraphConstructor():
    ...
    def _get_path_from_paths(self, paths):
        ...
        # in this version, we take the highest frequency -- the highest relations_score
        most_probable_path = []
        for path in paths:
            if most_probable_path == [] or self._relations_score(path) >= self._relations_score(most_probable_path):
...
#         if len(paths) > 1:
#             print(f"multiple 2-hop paths, {c_source} -> to -> {c_target} = {paths}. final paths is ={self._get_path_from_paths(paths)}")
#             import pdb
#             pdb.set_trace()
        paths = self._get_path_from_paths(paths)
        self.lfu_cache.set('{}_{}'.format(c_source, c_target), paths)
