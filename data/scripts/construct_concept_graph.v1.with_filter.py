...
def remove_duplicate_lists(list_of_lists):
    unique_lists = set(tuple(inner_list) for inner_list in list_of_lists)
...
class GraphConstructor():
    ...
    def _add_relations(self, head, rel, tail, weight):
        try:
    ...
    def load_c2r_vocab(self):
        ...
        with open(self.c2r_vocab_path, 'r') as csv_file:
            ...
            for indice, row in enumerate(csv_reader):
                ...
                try:
                    self._add_relations(head, rel, tail, weight)
                except:
                    import pdb
                    pdb.set_trace()
                
                if rel in self.SYMMTERIC_RELATIONS:
                    self._add_relations(tail, rel, head, weight)
    ...
    def _get_path_weight_score(self, path):
        score = 1.0
        ...
        for i in range(1, len(path), 2):
            ...
            tail = path[i+1]
            key = f"{head}__{rel}__{tail}"
            score *= self.relation_weight_score[key]
    ...
    def _extract_concept_relation(self, c_source, c_target):
        ...
        # using mid-in-the-middle algorithm

        # traverse from the source first
        reachable_concepts_from_head = {}
        queue_path = queue.Queue()
        queue_path.put([[c_source], 0])
        ...
        if c_source in self.direct_connections:
            for next_concept in list(set(self.direct_connections[c_source])):
                ...
                for next_rel in self.direct_connections[c_source][next_concept]:
                    ...
                    if next_rel == 'FormOf':
                        queue_path.put([[c_source, next_rel, next_concept],0])
        

        # traverse for 1-hop
        while not queue_path.empty():
            cur_path, cur_hop = queue_path.get()

            if cur_path[-1] == c_target:
                all_possible_paths.append(cur_path)
                continue

            if cur_hop == 1:
                if cur_path[-1] not in reachable_concepts_from_head:
                    reachable_concepts_from_head[cur_path[-1]] = []
                reachable_concepts_from_head[cur_path[-1]].append(cur_path)
                continue    

            prev_concept = cur_path[-1]
            if prev_concept in self.direct_connections:
                for next_concept in list(set(self.direct_connections[prev_concept])):
                    if next_concept not in self.concept2id:
                        continue
                    for next_rel in self.direct_connections[prev_concept][next_concept]:
                        if next_rel not in self.rel2id:
                            continue

                        # if next_concept not in cur_path:
                        next_path = copy.deepcopy(cur_path)
                        next_path.extend([next_rel, next_concept])
                        queue_path.put([next_path, cur_hop + 1])
        

        # traverse from target
        queue_path = queue.Queue()
        queue_path.put([[c_target], 0])

        while not queue_path.empty():
            cur_path, cur_hop = queue_path.get()

            if cur_path[-1] == c_source:
                cur_path.reverse()
                all_possible_paths.append(cur_path)
                continue
            ...
            if cur_hop == 1:
                prev_concept = cur_path[-1]
                if prev_concept in reachable_concepts_from_head:
                    cur_path.reverse()
                    cur_path = cur_path[1:]
                    for path in reachable_concepts_from_head[prev_concept]:
            ...
            prev_concept = cur_path[-1]
            if prev_concept in self.reverse_direct_connections:
                for next_concept in list(set(self.reverse_direct_connections[prev_concept])):
                    if next_concept not in self.concept2id:
                        continue
                    for next_rel in self.reverse_direct_connections[prev_concept][next_concept]:
                        if next_rel not in self.rel2id:
                            continue
        ...
        if len(all_possible_paths) > 0:
            print(f"PATHS between {c_source} and {c_target}")
            for id, path in enumerate(all_possible_paths):
                print(f"{path}, score={self._get_path_weight_score(path)}")
            print("======")

            all_possible_paths = [path for path in all_possible_paths if self._get_path_weight_score(path) >= 0.9]

            for path in all_possible_paths:
                paths.append([self.rel2id[rel] for step, rel in enumerate(path) if step % 2 == 1])
                

            import pdb
            pdb.set_trace()

        paths = remove_duplicate_lists(paths)
...
#                     print('==============================')
                ...
                if len(relations) > 0:
                    for relation in relations:
                        ...
                        rel_ids.append(relation)
                        # print(f"relation between {idx_head} and {idx_tail}, {relation}")
...
def main():
    ...
    graph_constructor = GraphConstructor(args)
    graph_constructor._extract_concept_relation("become", "remain")
