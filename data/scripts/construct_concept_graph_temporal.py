...
#         self.load_c2r_vocab()
    ...
    def _make_temporal_connection(self, head_info, tail_info):
        ...
        if (c_head.lower().startswith('for') or c_head.lower().startswith('over') or c_head.lower().startswith('within')) and (c_tail.lower().startswith('between') or c_tail.lower().startswith('from')):
            value = None
            try:
                value = w2n.word_to_num(c_head)
            except:
                value = re.findall('(\d+)', c_head)
                if len(value) > 0:
                    value = int(value[0])
            if value is None:
                return []
            
            tail_values = re.findall(r'([1-9][0-9]{3})', c_tail, re.IGNORECASE)
            tail_values = [int(x) for x in tail_values]
            if len(tail_values) > 1 and value == abs(tail_values[0] - tail_values[1]):
                return ['DurationOf']
            else:
                return []
        
        head_months = re.findall(r'(january|febuary|march|april|may|june|july|august|september|october|november|december)', c_head.lower())
        head_years = re.findall(r'([1-9][0-9]{3})', c_head.lower())
        head_years = [int(x) for x in head_years]
        
        tail_months = re.findall(r'(january|febuary|march|april|may|june|july|august|september|october|november|december)', c_tail.lower())
        tail_years = re.findall(r'([1-9][0-9]{3})', c_tail.lower())
        tail_years = [int(x) for x in tail_years]
        
        if len(tail_years) == 0 or len(head_years) == 0:
            return []
        
        if c_head.startswith('between') or c_head.startswith('from') or c_tail.startswith('between') or c_tail.startswith('from'):
            head_min, head_max = min(head_years), max(head_years)
            tail_min, tail_max = min(tail_years), max(tail_years)
            
            if not (head_min > tail_max or head_max < tail_min):
                return ['Overlap']
        
        same = True
        if tail_years[0] != head_years[0]:
            same=False
        if len(head_months)>0 and len(tail_months)>0 and head_months[0] != tail_months[0]:
            same=False
        if same:
            return ['IsSame']
        
        if int(head_years[0]) > int(tail_years[0]):
            return ['After']
        elif int(head_years[0]) < int(tail_years[0]):
            return ['Before']
        if len(head_months)>0 and len(tail_months)>0 and months.index(head_months[0].lower()) > months.index(tail_months[0].lower()):
            return ['After']
        elif len(head_months)>0 and len(tail_months)>0 and months.index(head_months[0].lower()) < months.index(tail_months[0].lower()):
            return ['Before']
        return []
        ...
        for idx_head, head in enumerate(tokens):
            for idx_tail, tail in enumerate(tokens):
                if idx_head == idx_tail:
                    continue
                relations = []
                if concepts[idx_head] >= temporal_argument_id and concepts[idx_tail] >= temporal_argument_id:
                    relations = self._make_temporal_connection((idx_head, None, tokens[idx_head]), (idx_tail, None, tokens[idx_tail]))
                elif concepts[idx_head] < temporal_argument_id and concepts[idx_tail] < temporal_argument_id:
...
def worker(args, batch, n_batch):
    ...
    with open('{}_{}.json'.format(args.output, batch), 'w') as f_out:
        for instance in tqdm(input_data[s_id:e_id]):
            if 'claim' not in instance:
                print(instance)
            claim = instance['claim']
            evi_list = list()
            for evidence in instance['predicted_evidence'][:7]:
                if evidence[1] == "":
                    continue
                try:
