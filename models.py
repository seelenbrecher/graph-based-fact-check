...
class inference_model(nn.Module):
    ...
    def initialize_node(self, token_level_bert, t2c_tensor, tpool_tensor, graph_tensor):
        ...
        edge_indices = edge_indices.reshape(-1, 2, n_rel)
        rel_indices = rel_indices.reshape(-1, n_rel)
        graph_inp_mask = graph_inp_mask.view(-1, n_rel)
        
        rel_indices = rel_indices.masked_fill_((1-graph_inp_mask).bool(), 0)
    ...
    def forward(self, inputs, test=False, roberta=False):
        ...
        inputs = torch.cat((inputs.unsqueeze(1), inputs_hiddens), dim=1).mean(dim=1)
        ...
        inputs_att = inputs_att.max(dim=1)[0]
