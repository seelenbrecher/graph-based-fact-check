...
class inference_model(nn.Module):
    def __init__(self, bert_model, concept_model, args):
        ...
        self.proj_word_att = nn.Linear(self.kernel, 1)
        self.proj_word_claim_evi = nn.Sequential(
            Linear(self.bert_hidden_dim * 4, self.bert_hidden_dim),
            ReLU(True),
        )
        self.word_level_pred = nn.Sequential(
            Linear(self.bert_hidden_dim, self.num_labels),
            nn.Tanh()
        )
        
        self.pred_bias = nn.Parameter(torch.Tensor(1))
    ...
    def word_level_predictor(self, inputs_hiddens, mask_claim, mask_evidence):
        mask_claim = mask_claim.view([-1, self.evi_num, self.max_len])
        mask_evidence = mask_evidence.view([-1, self.evi_num, self.max_len])
        inputs_hiddens = inputs_hiddens.view([-1, self.evi_num, self.max_len, self.node_dim])

        claim_hidden = torch.mean(inputs_hiddens, 1)
        claim_hidden_norm = claim_hidden.unsqueeze(1)
        claim_hidden_norm = claim_hidden_norm.repeat(1, self.evi_num, 1, 1)
        claim_hidden_norm = F.normalize(claim_hidden_norm, p=2, dim=-1)
        claim_mask = mask_claim.view(-1, self.evi_num, self.max_len)
        
        evi_hidden = F.normalize(inputs_hiddens, p=2, dim=-1)

        att_score = self.get_att_score_word_level(
            claim_hidden_norm.view(-1, self.max_len, self.node_dim),
            evi_hidden.view(-1, self.max_len, self.node_dim),
            claim_mask.view(-1, self.max_len),
            mask_evidence.view(-1, self.max_len)
        ) # (batch_size, self.max_len, self.max_len * self.evi_num, 1)
        
        claim_evi_hidden = inputs_hiddens.view(-1, self.max_len * self.evi_num, self.node_dim)
        claim_evi_hidden = claim_evi_hidden.repeat(self.max_len, 1, 1).view(self.max_len, -1, self.max_len * self.evi_num, self.node_dim)
        claim_evi_hidden = claim_evi_hidden.transpose(0, 1)
        claim_evi_hidden = (claim_evi_hidden * att_score).mean(2)
        
        denoise_inputs = torch.cat([claim_hidden, claim_evi_hidden, claim_hidden - claim_evi_hidden, claim_hidden * claim_evi_hidden], -1)
        denoise_inputs = self.proj_word_claim_evi(denoise_inputs)
        denoise_inputs = denoise_inputs.max(1)[0]
        pred = self.word_level_pred(denoise_inputs)
        pred = F.softmax(pred, dim=1)
        return pred
        
    
    def get_att_score_word_level(self, q_embed, d_embed, attn_q, attn_d):
        ...
        log_pooling_sum = torch.log(torch.clamp(pooling_value, min=1e-10))
        log_pooling_sum = self.proj_word_att(log_pooling_sum)
        
        attn_d = attn_d.repeat(1, self.max_len, 1, 1)
        log_pooling_sum = log_pooling_sum.masked_fill_((1 - attn_d).bool(), -1e4)
        log_pooling_sum = log_pooling_sum.view(-1, self.evi_num, self.max_len, self.max_len, 1)
        log_pooling_sum = log_pooling_sum.transpose(1, 2).reshape(-1, self.max_len, self.max_len * self.evi_num, 1)
        log_pooling_sum = F.softmax(log_pooling_sum, dim=2)
...
#         mask_evidence = seg_tensor.float() * mask_text
        ...
        evi_prob = torch.sigmoid(log_pooling_sum).squeeze(-1)
        
        # word level predictor
        word_level_prob = self.word_level_predictor(inputs_hiddens, mask_claim, mask_evidence)
        ...
        bias = F.sigmoid(self.pred_bias)
        prob = bias * prob + (1 - bias) * word_level_prob
        prob = torch.log(prob)
        return prob, evi_prob, class_prob
