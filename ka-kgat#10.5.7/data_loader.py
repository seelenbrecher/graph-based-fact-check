...
def tok2int_sent(sentence, tokenizer, max_seq_length, graph_constructor, roberta=False):
    ...
    tokens += padding
    c_start_id, c_end_id = 1, 1 + len(tokens_a) + 1 #exclusive
    if tokens_b:
        e_start_id, e_end_id = c_end_id, c_end_id + len(tokens_t) + 1 + len(tokens_b) + 1
    else:
    ...
    def next(self):
        ...
        if self.step < self.total_step:
            ...
            for step in range(len(inputs)):
                ...
                rel_ids, rel_seg = get_rel_inputs(self.tokenizer)
                rel_inp_ids.append(rel_ids)
                rel_segments.append(rel_seg)
            ...
            rel_segments_tensor = Variable(torch.FloatTensor(rel_segments)).view(-1, len(rel_segments[0]), len(rel_segments[0][0]))
