...
class inference_model(nn.Module):
    def __init__(self, bert_model, concept_model, args):
        ...
        graph_proj_dim = 4 * self.srl_lstm_hidden_dim # concatenate (clam + evi), self.node_dim each
    ...
    def refine_graph_with_ner(self, ner, ner_tagid, evi_srls, evi_tags):
        ...
        alignment_scores = alignment_scores.view(batch_size, self.evi_num, n_evi_srl, 1)
        
        zeros = torch.zeros((batch_size, self.evi_num, n_evi_srl, 1)).cuda() # for [PAD] idx in path
        alignment_scores = torch.cat((alignment_scores, zeros), 2)
                
        return alignment_scores #(batch_size, self.evi_num, n_ner, 1)
    ...
    def get_graph_reprs(self, path_reprs, node_scores=None, path_masks=None):
        batch_size, _, n_path, n_token, n_dim = path_reprs.shape

        path_reprs = path_reprs.view(-1, n_token, n_dim)
        
        if node_scores is not None:
            _, _, n_node, _ = node_scores.shape
            
            node_scores = node_scores.view(-1, n_node, 1)
            path_masks = path_masks.view(-1, n_path, n_token)
            
            scores = torch.cat([score[mask] for score, mask in zip(node_scores, path_masks)])
            path_reprs = path_reprs * scores
    ...
    def get_ner_important_scores(self, inputs, ner_reprs, ner_tags):
        ...
        ner_tags = ner_tags.view(batch_size, 1, -1)
        ner_tags = ner_tags.repeat(1, self.evi_num, 1)
        ner_tags = ner_tags.view(-1, n_ner)
        
        ner_tag_embed = self.srl_node_embed(ner_tags)
        ner_tag_scores = self.srl_node_proj(ner_tag_embed)
        ner_tag_scores = ner_tag_scores.squeeze(-1)

        scores = torch.bmm(ner_reprs, inputs.transpose(1, 2))
        scores = torch.sum(scores, dim=2)
        
        scores = F.softmax(scores * ner_tag_scores, dim=1)
        scores = scores.view(batch_size, self.evi_num, -1)

        return scores

    def get_class_pred_per_ner(self, inputs, claim_paths, evi_reprs, evi_paths, evi_path_masks):
        # get representations from paths
        claim_path_reprs = self._get_path_reprs(inputs, claim_paths)
        evi_path_reprs = self._get_path_reprs(inputs, evi_paths)

        claim_graph_reprs = self.get_graph_reprs(claim_path_reprs)
        evi_graph_reprs = self.get_graph_reprs(evi_path_reprs, evi_reprs, evi_path_masks)

        reprs = torch.cat((
            claim_graph_reprs, evi_graph_reprs, claim_graph_reprs - evi_graph_reprs, claim_graph_reprs * evi_graph_reprs), 
            dim=2
        )
    ...
    def predict_single_node(self, inputs, srl_tensor):
        ...
        claim_masks, claim_tags, claim_paths, claim_path_masks = claim_srl_input
        evi_masks, evi_tags, evi_paths, evi_path_masks = evi_srl_input
        ...
        refined_evi_reprs = refined_evi_reprs.permute(1, 2, 0, 3, 4) # (bs, evi_num, n_ner, n_bert_token, 1) represent node alignment scores
        ...
        for index in range(n_ner):
            ...
            class_pred, evi_completion = self.get_class_pred_per_ner(inputs, claim_paths, evi_repres, evi_paths, evi_path_masks)
        ...
        ner_important_scores = self.get_ner_important_scores(inputs, ner_node_reprs, ner_tagids)
    ...
    def initialize_node(self, token_level_bert, t2c_tensor, tpool_tensor, graph_tensor):
        ...
        if self.use_concept:
            ...
            if self.span_use_gat:
                edge_indices, rel_indices, graph_inp_mask  = graph_tensor
                batch_size, evi_num, _, n_rel = edge_indices.shape # _ = 2, source and target dim
                edge_indices = edge_indices.view(-1, 2, n_rel)
            
                token_level_concept, _ = self.span_gat((token_level_concept, edge_indices))
