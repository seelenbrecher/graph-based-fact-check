...
def add_ner_srl_args(parser):
    parser.add_argument('--ner_srl_train_path', default='../data/fever_srl_and_ner_1/train.json', type=str)
    parser.add_argument('--ner_srl_eval_path', default='../data/fever_srl_and_ner_1/eval.json', type=str)
    parser.add_argument('--ner_srl_test_path', default='../data/fever_srl_and_ner_1/test.json', type=str)
    parser.add_argument('--srl_n_node_types', default=29, type=int)
    
    # lstm
    parser.add_argument('--srl_lstm_hidden_dim', default=128, type=int)
    parser.add_argument('--srl_lstm_num_layers', default=2, type=int)
...
def pad_srl_paths(items, PAD_VAL=-1, EVI_LEN=5, MAX_PATH_LEN=30):
    """
...
def generate_srl_ner_input(srls_and_ners, EVI_LEN=5, MAX_SEQ_LENGTH=130):
    ...
    for srl_and_ner in srls_and_ners:
        ...
        batch_claim_path_masks.append(srl_and_ner['claim_path_masks'])
        ...
        batch_evi_path_masks.append(srl_and_ner['evi_path_masks'])
    ...
    pad_srl_tags(batch_claim_tags, EVI_LEN)
    pad_srl_paths(batch_claim_paths, MAX_SEQ_LENGTH-1, EVI_LEN)
    pad_srl_paths(batch_claim_path_masks, len(batch_claim_masks[0][0]), EVI_LEN)
    
    pad_srl_masks(batch_evi_masks, EVI_LEN)
    pad_srl_tags(batch_evi_tags, EVI_LEN)
    pad_srl_paths(batch_evi_paths, MAX_SEQ_LENGTH-1, EVI_LEN)
    pad_srl_paths(batch_evi_path_masks, len(batch_evi_masks[0][0]), EVI_LEN)
    
    ner_input = (batch_ner_masks, batch_ner_tag_ids)
    claim_srl_input = (batch_claim_masks, batch_claim_tags, batch_claim_paths, batch_claim_path_masks)
    evi_srl_input = (batch_evi_masks, batch_evi_tags, batch_evi_paths, batch_evi_path_masks)
    return ner_input, claim_srl_input, evi_srl_input