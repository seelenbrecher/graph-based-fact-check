...
def should_eval(global_step, eval_step, gradient_accumulation_steps):
    if global_step > 32000:
        eval_step /= 4
    return global_step % (eval_step * gradient_accumulation_steps) == 0
...
def train_model(model, ori_model, args, trainset_reader, validset_reader):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for index, data in enumerate(trainset_reader):
            ...
            if should_eval(global_step, args.eval_step, args.gradient_accumulation_steps):
